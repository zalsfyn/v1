...
if [ -f "/etc/systemd/system/zalsfynsshws.service" ]; then
clear
else
wget -q -O /usr/bin/proxy3.js "https://raw.githubusercontent.com/zalsfyn/v1/main/ssh/proxy3.js"
cat <<EOF> /etc/systemd/system/zalsfynsshws.service
[Unit]
...
wget -q -O /usr/bin/ssh-wsenabler "https://raw.githubusercontent.com/zalsfyn/v1main/ssh/sshws-true.sh" && chmod +x /usr/bin/ssh-wsenabler && /usr/bin/ssh-wsenabler
systemctl daemon-reload >/dev/null 2>&1
systemctl enable zalsfynsshws.service >/dev/null 2>&1
systemctl start zalsfynsshws.service >/dev/null 2>&1
...
PID=`ps -ef |grep -v grep | grep sshws |awk '{print $2}'`
if [[ ! -z "${PID}" ]]; then
systemctl stop zalsfynsshws.service
