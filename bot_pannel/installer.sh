...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/zalsfyn/otw/main/REG/ip > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/zalsfyn/otw/main/REG/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/zalsfyn/otw/main/REG/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
surat=$(curl -sS https://raw.githubusercontent.com/zalsfyn/otw/mainREG/authbot  | grep -w $ipes | awk '{print $4}'  )
if [[ "$surat" = "true" ]]; then
echo -ne
else
...
EOF
        ...
        fun_bot1() {
            [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
				wget -qO- https://raw.githubusercontent.com/zalsfyn/otw/main/bot_panel/ShellBot.sh >/etc/.maAsiss/.Shellbtsss
			}
